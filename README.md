...
The primary public site is [duekrio.pages.dev](https://duekrio.pages.dev/), a Cloudflare Pages Free direct-upload project. The Duekrio release and its live checks are documented in [hosting status](docs/HOSTING.md). GitHub push alone does not update this project; each release requires a separate hosted build and upload. The [security audit](docs/SECURITY_AUDIT.md), [legal/policy risk review](docs/LEGAL_RISK_REVIEW.md), [incident response checklist](docs/INCIDENT_RESPONSE.md), [design QA](docs/DESIGN_QA.md), and [brand check](docs/BRAND.md) record what has been checked and what remains unresolved. [Billing design](docs/BILLING.md) is a planning document; no payment account or live checkout is connected. These reviews are not guarantees of legal immunity or security.

The [Duekrio source repository](https://github.com/Akam1123/duekrio) has private vulnerability reporting, Dependabot alerts/security updates, secret scanning with push protection, and CodeQL default setup enabled. Main CI and CodeQL passed for the reviewed source release; GitHub reported no open code-scanning, Dependabot security, or secret-scanning alerts at that check. These are point-in-time results, not proof that the site has no vulnerabilities. See the security audit for evidence and remaining limits.
