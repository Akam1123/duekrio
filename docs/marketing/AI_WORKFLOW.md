...
**Status: on hold.** The user has explicitly said not to begin promotion yet. AI can prepare research organization, copy variants, editing, and analysis now. After a later explicit start instruction, the assistant will verify facts and execute the free channels it can access through existing authorized accounts. The user has no campaign tasks. No account setup, OAuth connection, or paid service is assumed. Nothing in this repository posts to social networks or sends marketing mail automatically, and prospect invoices must not be processed with AI.

## Approved fact card (paste into each generation request)

> Product: PromiseLedger, a public free prototype at https://akam1123.github.io/promiseledger/ . Audience: small US B2B service firms and bookkeepers. It imports a flat AR aging CSV, records invoice blockers, owner labels, next actions and customer promise dates, creates an email draft a person reviews and sends, and exports CSV/JSON backup. There is no account, cloud sync, automatic email, accounting API, billing, or verified customer outcome. Data is stored in the user's browser on that device. Users should export a backup and verify balances and paid status in their accounting system. Do not claim customers, revenue, payment recovery, time savings, compliance, security certification, or features beyond this card. The sample data is fictional.
...
1. **Start gate:** Do not publish, post, message, or schedule distribution before the user's later explicit start instruction. Then inspect the live product and which existing authorized channels are actually usable. Skip unavailable channels without assigning the user a task.
