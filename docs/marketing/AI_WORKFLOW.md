...
**Status: free-only launch and zero-cost promotion authorized after verified deployment.** Orvaket is a provisional name, not trademark-cleared. `https://orvaket.pages.dev/` is the intended new Cloudflare Pages URL; it has not yet been verified live at this update. Point-in-time checks belong in [HOSTING.md](../HOSTING.md). The previous origins remain migration fallbacks. A public contact email has been provided for the site, but delivery has not been verified; no paid plan or checkout is active. AI can help organize research, draft variants, edit, and analyze evidence. Before publication, the assistant will recheck brand, host, live URL, facts, and each free channel's rules. The user has no campaign tasks. No account setup, OAuth connection, or paid service is assumed. Nothing in this repository posts to social networks or sends marketing mail automatically, and prospect invoices must not be processed with AI.

## Approved fact card (paste into each generation request)

> Product: Orvaket (provisional name), a free browser app. Intended public URL: https://orvaket.pages.dev/ (verify the new Cloudflare Pages deployment before publication). Audience: small US B2B service firms and bookkeepers. It imports a flat AR aging CSV, records invoice blockers, owner labels, next actions and customer promise dates, creates an email draft a person reviews and sends, and exports CSV/JSON backup. A user may passphrase-encrypt a downloaded JSON backup and may opt in to encrypted local saves; default browser storage, CSV, and plain JSON remain readable. A prior readable browser copy remains until verified backup and explicit removal. There is no account, cloud sync, automatic email, accounting API, billing, or verified customer outcome. Data is stored in the user's browser on that device. Users should export a backup and verify balances and paid status in their accounting system. Do not claim customers, revenue, payment recovery, time savings, compliance, security certification, or features beyond this card. The sample data is fictional. Zero-cost promotion is authorized only after the renamed free site passes live verification.
...
1. **Start gate:** The user has authorized free, zero-cost distribution once the Orvaket site is verified live. First recheck the provisional brand screen, actual host/URL/product/contact copy, and existing authorized channels. Verify the `build:hosted` artifact and live pages match the final origin, and that core pages have consistent canonical, robots, and sitemap signals while retired kit pages remain out of search. Skip unavailable channels without assigning the user a task.
...
6. **Publish once where appropriate:** After live verification, the assistant can publish a useful guide on the existing owned site. A professional post or direct response is conditional on a relevant existing account, clear authority, and channel rules. Keep the exact published version and date in a private log. No social automation or mass messaging.
