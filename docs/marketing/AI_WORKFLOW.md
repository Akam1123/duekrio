...
**Status: verified free launch, zero-cost promotion authorized.** Orvaket is a provisional name, not trademark-cleared. The Cloudflare Pages release is live at [orvaket.pages.dev](https://orvaket.pages.dev/); point-in-time checks belong in [HOSTING.md](../HOSTING.md). The previous origins remain migration fallbacks. A public contact email is shown on the site, but delivery has not been verified; no paid plan or checkout is active. IndexNow accepted six URLs with HTTP 202 and the [GitHub v0.1.0 release](https://github.com/Akam1123/orvaket/releases/tag/v0.1.0) is public; neither proves people are using the app. AI can help organize research, draft variants, edit, and analyze evidence. Before each publication, the assistant will recheck brand, live URL, facts, and channel rules. The user has no campaign tasks. No account setup, OAuth connection, or paid service is assumed. Nothing in this repository posts to social networks or sends marketing mail automatically, and prospect invoices must not be processed with AI.

## Approved fact card (paste into each generation request)

> Product: Orvaket (provisional name), a free browser app. Public URL: https://orvaket.pages.dev/ (live release verified 26 September 2026; recheck before each new publication). Audience: small US B2B service firms and bookkeepers. It imports a flat AR aging CSV, records invoice blockers, owner labels, next actions and customer promise dates, creates an email draft a person reviews and sends, and exports CSV/JSON backup. A user may passphrase-encrypt a downloaded JSON backup and may opt in to encrypted local saves; default browser storage, CSV, and plain JSON remain readable. A prior readable browser copy remains until verified backup and explicit removal. There is no account, cloud sync, automatic email, accounting API, billing, or verified customer outcome. Data is stored in the user's browser on that device. Users should export a backup and verify balances and paid status in their accounting system. Do not claim customers, revenue, payment recovery, time savings, compliance, security certification, or features beyond this card. The sample data is fictional. Zero-cost promotion is authorized for the verified free release.
...
1. **Release and per-channel gate:** The user authorized free, zero-cost distribution and the Orvaket site passed live verification. Before each new asset or channel, recheck provisional brand limits, actual host/URL/product/contact copy, the `build:hosted` artifact if changed, and account authority/rules. Core pages have canonical, robots, and sitemap signals; retired kit pages remain `noindex`. Skip unavailable channels without assigning the user a task.
...
6. **Publish once where appropriate:** With the free site verified live, the assistant can publish or improve a useful guide on the owned site. A professional post or direct response is conditional on a relevant existing account, clear authority, and channel rules. Keep the exact published version and date in a private log. No social automation or mass messaging.
...
The live-verification gate has passed; each new generated asset still needs an individual check of product truth, live links, source relevance and date, spelling, accessibility, privacy, tone, and channel rules before publication. [Google's people-first guidance](https://developers.google.com/search/docs/fundamentals/creating-helpful-content) favors original, useful work; AI-assisted drafting is not a license to publish many thin pages. The maker should be candid if a reader would reasonably expect to know how content was produced.
