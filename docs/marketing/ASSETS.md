...
The user has explicitly said not to begin promotion yet. These are prepared drafts, not published posts or sent messages. Duekrio is a provisional name, not trademark-cleared. The [https://duekrio.pages.dev/](https://duekrio.pages.dev/) project has been created, but its rebranded release still needs upload and live verification. `[VERIFIED_LIVE_URL]` remains a deliberate publication placeholder: replace it with the then-current verified URL only after the user starts promotion and the brand, private contact route, page facts, and channel rules are rechecked. Do not publish any draft while the placeholder remains. “Free” describes current access to the prototype; it is not a forever-pricing promise. All examples below use fictional situations.
...
**Positioning line:** Know why an invoice is stuck, who owns the next step, and when to follow up.

**Hero option A:** Make every open invoice actionable.

**Hero support:** Import an AR aging CSV. Record the blocker, owner, next step, and payment promise in one focused board. Draft a follow-up you review before sending.

**Hero option B:** An AR follow-up list that remembers the reason.

**CTA:** Explore fictional sample invoices · Import your CSV
...
## Optional longer guide draft

The site already contains a weekly AR review checklist and overdue-email templates. Use this longer draft only if later buyer questions reveal a distinct need; otherwise improve the existing checklist. Do not publish a near-duplicate page to create another search URL.

**Title: How to turn an AR aging report into a weekly action list**

An accounts receivable aging report tells you which invoices are open and how long they have been outstanding. It rarely tells you *why* a particular invoice is still unpaid or who should resolve the issue. A weekly review becomes useful when each exception ends with one owner and one next action.

Start with a current export from your accounting system. Check that the file contains customer, invoice number, **remaining balance**, and due date. If the report includes both original invoice amount and remaining balance, use the remaining balance for follow-up. Keep the report date visible: a CSV is a snapshot, and an invoice could have changed since export.

Sort the open items into three working groups:

1. **Waiting on the customer:** They have the invoice and need to confirm a date or approve payment.
2. **Blocked by information:** A purchase order, timesheet, tax form, or corrected invoice is missing.
3. **Disputed:** The customer questions the scope, amount, or service. Resolve the dispute before another generic reminder.

For each invoice, write the smallest useful next step. “Follow up” is vague; “send revised PO reference to AP by Thursday” is a task someone can finish. Record one responsible person and a review date. If the customer promises to pay on a particular date, store the date and the source of the promise in a note. A promise is a commitment to check, not confirmed cash.

Consider a fictional example: invoice `INV-1042` has a $2,400 remaining balance and is 35 days overdue. The buyer's AP team says its PO number is missing. The owner is Maya, and the next action is “send corrected invoice with PO 8174 today; confirm receipt Friday.” A generic “Your invoice is overdue” email would miss the real blocker. The action list gives the team a way to finish the specific job.

Before sending a follow-up, compare the invoice with the current ledger and recent customer correspondence. Keep the email factual and short: invoice reference, current amount and due date, what you believe is blocking payment, and the one answer you need. A human should check the recipient, balance, and tone. Do not treat an exported board as the source of truth for paid status.
...
An AR aging report can tell you that an invoice is 35 days overdue. It usually cannot tell you that the buyer is waiting for a PO number.
...
### Post 2 — a practical checklist

Before the next overdue-invoice meeting, try this five-column list:

1. Remaining balance and due date
2. Reason payment is stuck
3. Person responsible for the next step
4. Specific next action and date
5. Customer promise, if one actually exists
...
### Community answer, only if a real question invites it

For a small team, I would start from a current AR aging export and separate “late but straightforward” invoices from exceptions. For each exception, record why it is stuck, one owner, one next action, and a date to check again. A dispute or missing PO usually needs a different message from a routine reminder. Reconcile against the ledger each week so an old CSV does not trigger a wrong follow-up. I built a local-first tool for this, but the same checklist works in a spreadsheet too. [Include the product link only if the community's rules and the conversation permit self-promotion.]
...
**Subject:** AR exception workflow — quick critique?
...
### Follow-up after an invited demo

**Subject:** The invoice follow-up example we discussed
...
### Partner introduction request to a known adviser

**Subject:** One introduction to a service firm with messy AR follow-up?

Hi [name],

I am testing a small tool for B2B teams whose aging report shows *what* is late but not *why* it is stuck. It imports a CSV and creates a board of blockers and next actions. The prototype is free and has no accounting integration or automatic email.
...
**0–7s:** “An aging report shows an overdue invoice. It does not always show why payment is stuck.” Show fictional CSV and its report date.

**7–20s:** Import the fictional file. Open an invoice blocked by a missing PO. Add the blocker, owner, and next action.

**20–32s:** Show a promise date and a draft follow-up. Say, “A person checks and sends the message.”
