...
The [ranked distribution experiments](marketing/CHANNEL_RANKING.md) specify where to test for qualified buyers after the free site and trust gates pass. The user has authorized those zero-cost experiments after live verification; no external distribution is recorded yet.
...
| 4. Later paid validation | Only after a separate decision to activate sales, a provider approves the real operator and product, and the distinct downloadable kit can be delivered, offer it at one transparent one-time test price. Do not charge for the free app or an unbuilt SaaS feature. | At least 3 of 5 qualified firms choose the same published kit price without a custom feature promise; this is a future experiment target, not a current result. |
...
| Paid demand | In a later paid phase, after the kit and merchant route are genuinely live, at least 3 of 5 qualified firms buy the same $39 one-time kit without custom promises. Treat any earlier stated intention as weaker evidence than an order. | Tests willingness to pay for the first distinct deliverable. Recurring SaaS demand remains untested. |
...
After 20 qualified interviews, revisit the ICP if fewer than 7 report the specific blocker-and-commitment pain. After five well-supported free pilots, reconsider the product if fewer than two return for a second follow-up cycle. Assess stated-price demand only when the later paid experiment is authorized and deliverable. Before stopping, separate product failure from poor lead quality or import defects; fix a concrete defect and rerun a small pilot once. Avoid extending the experiment indefinitely without a paid signal.

## Measuring value honestly

Capture a baseline during each pilot: open invoices, aging buckets, time spent on weekly follow-up, number of unresolved blockers, and number of commitments missed. Compare the same measures after two cycles. Count cash as recovered only when the buyer confirms payment in the ledger; do not attribute all paid invoices to Orvaket. Report self-reported time savings separately from verified payment outcomes. Ask permission before publishing any identifiable result.
