...
Test $29/month for one operator *only after* a distinct, working premium benefit exists and the product supports a repeatable workflow worth paying for. The first candidate is authenticated hosted backup and restore, subject to a new privacy and security review. A $59/month team plan is a later hypothesis that depends on shipping real accounts, permissions, and safe collaboration; the local MVP has no team access. A guided setup service is not the initial offer because it needs human delivery and may not be accepted by the chosen merchant of record. These are hypotheses, not current prices or existing billing features. Subscription value and entitlement for a local-only app also need validation. Compare willingness to pay with the buyer's actual cost of manual chasing and blocked cash; do not infer it from Chaser's price. The [billing plan](BILLING.md) records provider eligibility and payout constraints, including the owner's preference to decide when to withdraw available proceeds.
