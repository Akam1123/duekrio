# Incident response — Duenara early access
...
4. Check whether the issue could have exposed browser-local invoice data. A script executing on the site's origin can read plaintext `localStorage`; optional encrypted JSON backup files do not protect that live browser data. Readable JSON backups and CSV exports are plaintext, while encrypted JSON backups need their passphrase. Determine the earliest affected release, which pages were served, any unexpected outbound requests, and the plausible visitor set. The static app has no server-side invoice database, so do not claim it had one or that no user data could have been affected.
...
- GitHub [private vulnerability reporting](https://github.com/Akam1123/promiseledger/security/advisories/new) is enabled for software security findings. A private customer/privacy request channel and accountable operator details have **not yet been established**; public GitHub Issues are unsuitable for confidential customer reports. This remains a release limitation recorded in [LEGAL_RISK_REVIEW.md](LEGAL_RISK_REVIEW.md).
