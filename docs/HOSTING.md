...
The Duekrio `main` direct-upload release was built from source commit `44a52d44d71c6245615c473395aa52b01dfdbfb3` and published at [duekrio.pages.dev](https://duekrio.pages.dev/) on 25 September 2026. A live HTTP check returned 200 with the intended CSP, HSTS, `nosniff`, framing restriction, and `X-Robots-Tag: noindex, nofollow`. A live browser pass checked six pages, the 404 page, the sample copy action, canonical metadata, 320–1440 px layouts, accessibility automation, and browser errors. A full live encrypted-workspace regression also passed on a repeat run after earlier intermittent raw-file verification timeouts; that race is under review and will be fixed before the release is considered final.

The Duenara `main` compatibility direct upload was built from the same source commit `44a52d44d71c6245615c473395aa52b01dfdbfb3` and published at [duenara.pages.dev](https://duenara.pages.dev/) on 25 September 2026. Live browser checks passed the six content pages, 404, CSP/noindex headers, canonical metadata, sample copy action, 320–1440 px layouts, and accessibility automation. A fresh-browser sample backup exported on Duenara and restored on Duekrio. Existing browser storage on either origin was not read or removed.
