...
Browser `localStorage` belongs to an origin. GitHub Pages, Duenara, Duekrio, and Orvaket are four distinct storage origins. A user must open the address where the work was saved, unlock it if necessary, download a JSON backup, restore it at Orvaket, and verify the restored records before removing the old copy. CSV alone does not preserve the whole workspace. Do not automatically redirect or remove an old workspace. The Duekrio compatibility deployment now displays the migration notice and retains the old workspace for export.
