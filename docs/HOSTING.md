...
[Netlify Free](https://www.netlify.com/blog/introducing-netlify-free-plan/) is a second viable commercial-static-site host. It offers a root `*.netlify.app` address, HTTPS, and [custom response headers](https://docs.netlify.com/manage/routing/headers/), with a [hard free-plan usage limit](https://docs.netlify.com/manage/accounts-and-billing/billing/billing-for-credit-based-plans/credit-based-pricing-plans/). It is a fallback if Cloudflare onboarding fails, not an address already created here. Its [anonymous CLI deploy](https://docs.netlify.com/api-and-cli-guides/cli-guides/get-started-with-cli/) also expires after an hour unless an account holder signs in and claims it; no Netlify account or token is available on this machine. Provider-specific privacy copy and headers must be reviewed before switching.
...
Wrangler 4.140.0 was also checked on this machine with `wrangler whoami`; it reported **not authenticated**. Cloudflare's [temporary Worker deployments](https://developers.cloudflare.com/workers/platform/claim-deployments/) can give an agent an unauthenticated preview, but Cloudflare deletes it after 60 minutes unless the intended account holder signs in and claims it. Creating that preview also entails accepting Cloudflare's policies. It is therefore a QA tool, not the stable address requested here. A free DNS subdomain pointing at this GitHub Pages prototype would still inherit [GitHub Pages' commercial-hosting limitation](https://docs.github.com/en/pages/getting-started-with-github-pages/github-pages-limits/) and its inability to send our planned response headers; changing the visible URL alone would not solve the production problem.
